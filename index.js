...
app.get('/movies/:id', async function (req, res) {
  //db.movie.findOne{id:101};
  const {id}=request.params;
  console.log(request.params,id);
  const movie= await client.db("b36wd")
  collection("movie.movie").findOne({id:id});
...
app.post("/movies",express.json(),function(request,response)
{
  ...
  const result=await client.db("b36wd").collection("movie.movie").inserMany(data);
